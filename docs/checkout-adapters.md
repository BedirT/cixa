...
The adapter permits only an exact approved HTTPS navigation chain and owner-configured hosted-field processor origins. It rejects private or reserved network destinations, non-hosted payment forms, changed visible totals, currencies, fulfillment profiles, recurring, trial, or stored-card indicators, and missing sanitized provider references. Payment fields are filled only in a cross-origin iframe whose origin is explicitly configured. The adapter configuration, Node executable, and adapter script must be absolute, owner-controlled, regular non-symlink files that are not group or world writable.
