...
Initialize helper key material with `cixa init-helper --helper-dir DIR`. Start the one-operation helper with `cixa secret-helper`, piping one JSON secret object on stdin from an owner-only terminal or credential-store bridge. The socket must use an absolute path beneath a root- or owner-controlled non-writable directory chain. The helper authenticates the broker through Unix peer credentials, and the broker independently requires the connected helper peer to use the owner UID before sending the grant. The helper atomically redeems that signed grant across workers and restarts, returns at most 4096 bytes once, zeroes its buffer, removes its socket, and exits. Grants are bound to the helper instance and broker UID and expire after five minutes. Rotate by replacing the private helper directory only when no handoff is active.

Run `execute-handoff` with absolute paths for Node, `packages/checkout-playwright/dist/index.js`, and the private adapter config, plus the helper socket, key, and ID files. Stop `cixa serve` first; failure to acquire the exclusive data lock prevents browser launch. Payment material is accepted only by the helper on stdin, never in arguments, config, environment variables, output, or broker state.
