...
Cixa accepts at most 10,000 purchase intents in one treasury and 2,000 per agent. Ordinary audit actions stop at 100,000 events, while emergency-stop events are exempt so the primary safety control remains available. Purchase requests are limited to 4 KiB and individual redirect URLs to 2 KiB. Requests above an agent's per-minute rate are rejected before an intent is stored, with one coalesced audit event per minute. These limits keep a compromised agent from growing `state.json` or dashboard responses without bound. Before a treasury reaches a quota, stop agents, keep an owner-only backup of the complete data directory and audit export, then initialize a new data directory and issue new capabilities. Do not delete individual records from an active treasury because that invalidates its authenticated state and audit chain.
