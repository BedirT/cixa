...
const binary = join(root, "target", "debug", "treasury");
const directory = await mkdtemp(join(tmpdir(), "agent-treasury-mcp-"));
...
const socketPath = join(directory, "treasury.sock");
...
try {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      TREASURY_AGENT_TOKEN_FILE: agentFile,
    },
  });
  ...
  assert.equal(names.includes("treasury_get_status"), true);
  assert.equal(names.includes("treasury_get_budget"), true);
  assert.equal(names.includes("owner_set_emergency_stop"), false);
  const status = await client.callTool({ name: "treasury_get_status", arguments: {} });
