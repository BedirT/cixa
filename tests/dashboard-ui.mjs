...
try {
  ...
  const ownerToken = (await readFile(ownerFile,"utf8")).trim();
  const overview = await rpc(ownerSocket, ownerToken, {type:"owner_get_dashboard"});
  const agentId = overview.agents.find((agent) => agent.name === "Research Runner").id;
  await rpc(ownerSocket, ownerToken, {type:"owner_set_agent_mode",agent_id:agentId,mode:"bounded_autonomous"});
  await rpc(ownerSocket, ownerToken, {type:"owner_arm_agent_session",agent_id:agentId,ttl_secs:600});
  const heldRequest = purchase("pending-hold",500,"merchant.example.test"); heldRequest.scenario = "delayed_settlement";
  const heldIntent = await rpc(agentSocket, agentToken, {type:"create_purchase_intent",request:heldRequest});
  const heldExecution = await rpc(agentSocket, agentToken, {type:"execute_purchase_intent",intent_id:heldIntent.id});
  assert.equal(heldExecution.status,"provider_pending");
  await page.getByRole("link", {name:/Today/}).first().click();
  await page.getByRole("button", {name:"Refresh"}).click();
  const usageMetric = page.locator("#today-metrics .metric-card").filter({hasText:"Used or held in the last 24 hours"});
  await usageMetric.getByText("CA$5.00", {exact:true}).waitFor();
  await usageMetric.getByText("Across 1 agent", {exact:true}).waitFor();
  await rpc(ownerSocket, ownerToken, {type:"owner_reconcile",intent_id:heldIntent.id,outcome:"declined",provider_reference:"browser-pending-ref"});
  await rpc(ownerSocket, ownerToken, {type:"owner_set_agent_mode",agent_id:agentId,mode:"approval_required"});
  ...
  await page.getByRole("button", {name:"Refresh"}).click();
  await page.getByRole("button", {name:"All",exact:true}).click();
  await page.getByText("Showing 28 of 29 attempts", {exact:true}).waitFor();
  await page.getByRole("button", {name:"Load older attempts"}).click();
  await page.getByText("Showing 29 of 29 attempts", {exact:true}).waitFor();
  await page.getByRole("button", {name:"Refresh"}).click();
  await page.getByText("Showing 29 of 29 attempts", {exact:true}).waitFor();
