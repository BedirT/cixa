...
let csrf = "";
const state = { overview: null, checkout: null, audit: null, ledgerTransactions:[], ledgerCursor:null, ledgerTotal:0, auditCursor:null, route: "today", ledgerFilter: "all", search: "", busy: false, dialogAction: null, lastFocus: null, lastSuccessfulRefresh: null, offline:false };
...
async function refresh() {
  try {
    ...
    try { state.checkout = await api("/api/checkout"); }
    catch { state.checkout = null; }
...
function renderChrome() {
  ...
  const warnings = data.unsafe_modes.map((mode) => mode.includes("controlled checkout") ? "Controlled card checkout is on. Results stay unconfirmed until you reconcile them against KOHO." : mode.includes("manual prepaid-card") ? "Manual card mode is on. You will finish each checkout and confirm what happened." : humanReason(mode));
...
function renderToday() {
  ...
  $("#today-heading").textContent = pendingCount ? "Good morning." : "All quiet.";
  $("#today-summary").textContent = data.emergency_stop ? "Spending is stopped. Nothing can be bought until you start it again." : pendingCount ? `${pendingCount === 1 ? "One thing needs you" : `${pendingCount} things need you`}, and everything else is being handled quietly.` : "Nothing needs a decision. Cixa is reading every checkout as it comes in.";
...
function todayClear() { return node("div", { class:"today-clear" }, [node("strong", { text:"Nothing needs a decision." }), node("span", { text:"Cixa keeps reading every checkout in the background." })]); }
...
function renderCheckout() {
  const checkout=state.checkout;const badge=$("#payment-session-badge");const stop=$("#payment-session-stop");
  if(!checkout){badge.textContent="Unavailable";badge.className="state-badge";stop.disabled=true;replace($("#checkout-runtime-status"),[node("p",{text:"The local checkout runtime could not be reached. Refresh after starting the owner console with checkout support."})]);replace($("#checkout-profile-list"),empty("Checkout profiles are temporarily unavailable."));return;}
  const session=checkout.payment_session;badge.textContent=session.active?"Armed":"Not armed";badge.className=`state-badge${session.active?" success":""}`;stop.disabled=!session.active;
  const browserField=$("#checkout-profile-form").elements.browser_executable;if(!browserField.value&&checkout.suggested_browser_executable)browserField.value=checkout.suggested_browser_executable;
  replace($("#checkout-runtime-status"),[node("div",{class:"setup-status-copy"},[node("strong",{text:checkout.runtime_initialized?"Secure helper ready":"Secure helper not initialized"}),node("span",{text:session.active?`Session ends ${when(session.expires_at)} · up to ${session.max_operations} checkouts`:`${checkout.profiles.length} approved ${checkout.profiles.length===1?"profile":"profiles"} · no card details in storage`})])]);
  replace($("#checkout-profile-list"),checkout.profiles.length?checkout.profiles.map((profile)=>node("article",{class:"profile-row"},[node("div",{},[node("strong",{text:profile.merchant_domain}),node("span",{text:`${profile.profile_id} · ${profile.processor_origins.length} processor ${profile.processor_origins.length===1?"origin":"origins"}`})]),node("div",{class:"profile-row-actions"},[node("span",{class:"state-badge success",text:"Approved"}),node("button",{type:"button",class:"quiet-button compact-button",text:"Remove",onclick:()=>confirmAction({title:`Remove ${profile.merchant_domain}?`,copy:"Controlled checkout will stop for this merchant as soon as the profile is removed.",label:"Remove profile",danger:true,action:()=>post("/api/checkout/profiles/delete",{profile_id:profile.profile_id},"Checkout profile removed.")})})])])):empty("No checkout profiles yet. Add one for each merchant agents may pay."));
}
function renderTrust() {
  ...
  $("#provider-form").elements.autonomous_checkout.checked=Boolean(card?.autonomous_checkout);
  ...
  if (!state.audit) { $("#audit-verification").textContent = "Audit history is temporarily unavailable. Other owner controls still work."; replace($("#audit-list"), empty("Refresh to try loading audit history again.")); }
    ...
    replace($("#audit-list"), state.audit.entries.length ? state.audit.entries.map((entry) => node("article", { class:"audit-entry" }, [node("div",{class:"audit-entry-head"},[node("span",{class:"audit-marker",attrs:{"aria-hidden":"true"}}),node("div",{},[node("h3", { text:title(entry.action) }),node("p", { text:`${entry.actor} · ${when(entry.at)}${entry.intent_id ? ` · ${entry.intent_id}` : ""}` })])]),node("details", {}, [node("summary", { text:"Technical evidence" }), node("p", { text:`Sequence ${entry.sequence} · hash ${entry.hash} · previous ${entry.previous_hash}` }), node("pre", { text:JSON.stringify(entry.details, null, 2) })])])) : empty("No audit events yet."));
...
function openPolicy(agent,policy) {
  ...
  const fields=specs.map(([key,label])=>{const input=node("input",{type:"number",min:"0.01",step:"0.01",value:(policy[key].minor/100).toFixed(2),required:true,dataset:{policyKey:key},attrs:{"aria-label":label}});return node("label",{class:"policy-limit-field"},[node("span",{class:"policy-field-label"},[label,node("small",{text:policy.primary_currency})]),input]);});
  const booleanInput=(key,label)=>node("label",{class:"checkbox-row"},[node("input",{type:"checkbox",checked:policy[key],dataset:{policyBoolean:key}}),label]);
  const merchantGuard=booleanInput("require_approval_for_new_merchants","New merchants need approval");merchantGuard.classList.add("policy-guardrail");
  const permissions=[["allow_recurring","Recurring charges"],["allow_trials","Trials"],["allow_stored_card","Stored cards"],["allow_tips","Tips"],["allow_preauthorization","Preauthorizations"],["allow_installments","Installments"]].map(([key,label])=>booleanInput(key,label));
  const limitsSection=node("section",{class:"policy-section"},[node("div",{class:"policy-section-heading"},[node("h3",{text:"Spending limits"}),node("p",{text:"Amounts are in the policy currency."})]),node("div",{class:"policy-limit-grid"},fields)]);
  const rulesSection=node("section",{class:"policy-section"},[node("div",{class:"policy-section-heading"},[node("h3",{text:"Checkout rules"}),node("p",{text:"Choose what this agent may use at checkout."})]),merchantGuard,node("div",{class:"policy-permission-grid"},permissions)]);
  confirmAction({title:`${agent.name} policy`,eyebrow:"Agent policy",variant:"policy",copy:"Set the boundaries Cixa checks before this agent can spend.",custom:[node("div",{class:"policy-editor-body"},[limitsSection,rulesSection])],label:"Save policy",action:()=>{const next=structuredClone(policy);$$('[data-policy-key]').forEach((input)=>{next[input.dataset.policyKey]={minor:Math.round(Number(input.value)*100),currency:policy.primary_currency};});$$('[data-policy-boolean]').forEach((input)=>{next[input.dataset.policyBoolean]=input.checked;});return post("/api/policies/update",{agent_id:agent.id,policy:next},"Policy saved.");}});
}
...
function confirmAction({title:heading,copy,facts=[],custom=[],label,danger=false,action,eyebrow="Owner confirmation",variant="confirmation"}) {
  state.dialogAction=action; state.lastFocus=document.activeElement; const dialog=$("#action-dialog");const form=$("#dialog-form");dialog.classList.toggle("policy-dialog",variant==="policy");form.classList.toggle("policy-editor",variant==="policy");$("#dialog-eyebrow").textContent=eyebrow;$("#dialog-title").textContent=heading; const contents=[node("p",{text:copy})]; if(facts.length)contents.push(node("div",{class:"fact-list"},facts.map(([key,value])=>fact(key,value)))); contents.push(...custom);replace($("#dialog-body"),contents);enhanceSelects($("#dialog-body"));const confirm=$("#dialog-confirm");confirm.textContent=label;confirm.className=danger?"secondary-button":"primary-button";dialog.showModal();
}
...
$("#provider-form").addEventListener("submit",(event)=>{event.preventDefault();const form=new FormData(event.currentTarget);const autonomous=form.get("autonomous_checkout")==="on";confirmAction({title:autonomous?"Enable controlled checkout?":"Save this KOHO setup?",copy:autonomous?"Agents may complete checkout only for your approved merchant profiles, within every standing limit, and only while you arm a short-lived payment session. You will still reconcile the result in KOHO.":"Cixa stores the reference and masked last four, not card credentials. The balance is based on your confirmation.",label:autonomous?"Enable and save":"Save setup",action:()=>post("/api/provider/manual",{credential_reference:String(form.get("credential_reference")),provider_kind:String(form.get("provider_kind")),last_four:String(form.get("last_four")),balance:{minor:Math.round(Number(form.get("balance"))*100),currency:String(form.get("currency")).toUpperCase()},balance_status:String(form.get("balance_status")),balance_ttl_secs:Number(form.get("balance_ttl_minutes"))*60,autonomous_checkout:autonomous},"KOHO setup saved.")});});
$("#payment-session-form").addEventListener("submit",async(event)=>{event.preventDefault();const element=event.currentTarget;const form=new FormData(element);try{await post("/api/checkout/session/arm",{pan:String(form.get("pan")).replace(/[^0-9]/g,""),expiry:String(form.get("expiry")),cvv:String(form.get("cvv")),cardholder:String(form.get("cardholder")).trim(),ttl_secs:Number(form.get("ttl_secs")),max_operations:Number(form.get("max_operations"))},"Payment session armed.");}catch{}finally{element.elements.pan.value="";element.elements.expiry.value="";element.elements.cvv.value="";element.elements.cardholder.value="";}});
$("#payment-session-stop").addEventListener("click",()=>post("/api/checkout/session/stop",{},"Payment session ended.").catch(()=>{}));
$("#checkout-profile-form").addEventListener("submit",async(event)=>{event.preventDefault();const form=new FormData(event.currentTarget);const lines=(name)=>String(form.get(name)).split(/\r?\n/).map((value)=>value.trim()).filter(Boolean);const selectors={finalTotal:String(form.get("final_total_selector")),currency:String(form.get("currency_selector")),fulfillment:String(form.get("fulfillment_selector")),items:String(form.get("items_selector")),recurring:String(form.get("recurring_selector")),trialAutoRenew:String(form.get("trial_selector")),storedCard:String(form.get("stored_card_selector")),tipMinor:String(form.get("tip_selector")),preauthorization:String(form.get("preauthorization_selector")),installments:String(form.get("installments_selector")),paymentFrame:String(form.get("iframe_selector")),pan:String(form.get("pan_selector")),expiry:String(form.get("expiry_selector")),cvv:String(form.get("cvv_selector")),cardholder:String(form.get("cardholder_selector")),submit:String(form.get("submit_selector"))};try{await post("/api/checkout/profiles",{profile_id:String(form.get("profile_id")),merchant_domain:String(form.get("merchant_domain")).toLowerCase(),browser_executable:String(form.get("browser_executable")),allowed_navigation_origins:lines("allowed_navigation_origins"),allowed_processor_origins:lines("allowed_processor_origins"),selectors,timeout_ms:Number(form.get("timeout_ms"))},"Checkout profile saved.");$("#profile-editor").open=false;}catch{}});
...
function applyTheme(theme,persist=false) { const dark=theme==="dark";document.documentElement.dataset.theme=dark?"dark":"light";if(persist)localStorage.setItem("cixa-theme",dark?"dark":"light");const label=dark?"Use light theme":"Use dark theme";const button=$("#theme-button");button.setAttribute("aria-label",label);button.title=label;button.querySelector("use").setAttribute("href",dark?"#icon-sun":"#icon-moon"); }
$("#theme-button").addEventListener("click",()=>applyTheme(document.documentElement.dataset.theme==="dark"?"light":"dark",true));
applyTheme(localStorage.getItem("cixa-theme")==="dark"?"dark":"light");
enhanceSelects();
navigate();
