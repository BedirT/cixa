...
function renderToday() {
  ...
  replace($("#today-metrics"), [metric("Spent today", money({ minor:spent, currency }), `Across ${active.length} active ${active.length === 1 ? "agent" : "agents"}`, allowance ? spent / allowance * 100 : 0), metric("Brought in", money(broughtIn), waitingIncome.minor ? `${money(waitingIncome)} more waiting on you` : "All confirmed by you", broughtIn.minor ? 100 : 0, "green"), metric("Still allowed today", money({ minor:remaining, currency }), "What active agents can use right now", allowance ? remaining / allowance * 100 : 0, "neutral")]);
