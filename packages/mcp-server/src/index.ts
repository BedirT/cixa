...
function createServer() {
  ...
  server.registerTool("cixa_execute_purchase_intent", { description: "Submit an authorized intent exactly once. A timeout or unknown/reconciliation state is ambiguous: read the intent, tell the owner, and never retry execution.", inputSchema: idInput }, (input) => safe(() => client.executePurchaseIntent(input.intent_id)));
